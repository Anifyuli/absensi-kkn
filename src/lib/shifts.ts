...
export const SHIFT_CONFIGS: ShiftConfig[] = [
  { id: 1, nama: 'Pagi',  icon: '🌅', jam: '09:00', hour: 9,  minute: 0, color: 'violet', windowMinutes: 240, startHour: 0, endHour: 14 },
  { id: 2, nama: 'Siang', icon: '☀️',  jam: '14:00', hour: 14, minute: 0, color: 'amber',  windowMinutes: 420, startHour: 14, endHour: 21 },
  { id: 3, nama: 'Malam', icon: '🌙',  jam: '21:00', hour: 21, minute: 0, color: 'sky',    windowMinutes: 780, startHour: 21, endHour: 24 },
]
...
export function getActiveShift(now: Date): ActiveShift | null {
  ...
  for (const cfg of SHIFT_CONFIGS) {
    // Handle midnight wrap-around for night shift
    if (cfg.id === 3 && currentHour >= cfg.startHour) {
      // Night shift: from 21:00 to 23:59
      return { config: cfg, status: 'hadir', menitSelisih: 0 }
    } else if (cfg.id === 3 && currentHour < 9) {
      // Night shift: from 00:00 to 08:59 next day
      return { config: cfg, status: 'hadir', menitSelisih: 0 }
...
export function getShiftColorClasses(color: string): {
  ...
  const map: Record<string, { badge: string; border: string; text: string; bg: string; dot: string }> = {
    violet: {
      badge: 'badge-shift-1',
